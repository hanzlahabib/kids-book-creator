...
import { Plus, Wand2, FileStack, Download, BookOpen, Coins } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [userStats, setUserStats] = useState<UserStats>({ credits: 0, planName: "Free" });
  ...
  const fetchData = async () => {
    try {
      const [statsRes, subRes] = await Promise.all([
        fetch("/api/stats"),
        session?.user ? fetch("/api/subscription") : Promise.resolve(null),
      ]);
          ...
          <Card>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div>
                  <p className="text-2xl font-bold">{userStats.credits}</p>
                  <p className="text-xs text-muted-foreground">{userStats.planName} plan</p>
                </div>
              )}
            </CardContent>
          </Card>
